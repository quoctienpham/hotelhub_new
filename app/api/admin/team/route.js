...
export async function POST(req) {
  ...
  // Extracting the "name"  position, image   field from the request body
  const { name, position, image } = body;

  try {
    // Creating a new Team in the Team collection with the provided name

    const team = await Team.create({ name, position, image });
    console.log("team-->", team);

    // Returning the created Team as a JSON response
    return NextResponse.json(team);
  } catch (err) {
    // If an error occurs, returning a 500 error with the error message
    return NextResponse.json({ err: err.message }, { status: 500 });
